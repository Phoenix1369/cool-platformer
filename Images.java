...
class Images 
{
	...
	public static final Color[] tintBG = {
		...
		new Color(0, 0, 0, 0), new Color(0, 255, 0, 100)
	};
	// Temporary Variables
	...
	public static void loadAll() throws IOException 
	{
		...
		// Tints an Image
		tint = new BufferedImage[2][4];
		for(int i = 0; i < tint.length; ++i)
			for(int j = 0; j < tint[i].length; ++j)
			{
				tint[i][j] = new BufferedImage(demo[i].getWidth(), demo[i].getHeight(), BufferedImage.TYPE_INT_ARGB);
				g2D = tint[i][j].createGraphics();
				g2D.drawImage(demo[i], 0, 0, null);
				g2D.setComposite(AlphaComposite.SrcAtop); // Composites (src in des) onto destination
				g2D.setColor(tintBG[j]); // Opacity: 100 / 255 Percent
				g2D.fillRect(0, 0, tint[i][j].getWidth(), tint[i][j].getHeight());
				g2D.dispose();
			}
	}	// end method loadAll
}	// end class Images
